...
*Date: 
*Description: Data sets given for Price and Production of Barley at State and District level
The aim of the code is to read in input files, perform data
cleaning, prepare a final dataset for analysis. The datasets used are from the 
US Department of Agriculture. 
• Barley production Data lists the barley production in bushels by agricultural
district. The agricultural district is an administrative division between the county
and state levels
• Barley price Data lists the mean price received by farmers per bushel of barley by
state.
...
/*
*3.1) For each year, computing the weighted average of price over all states 
with both price and production data in that year, where each state’s weight 
is its production in bushels in that year. Then, plotting this weighted average 
...
/*
3.2) Creating a summary table where the rows are specific states 
